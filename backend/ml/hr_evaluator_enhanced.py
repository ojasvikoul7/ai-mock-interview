...
Components: Content, Confidence, Structure, Sentiment
"""
...
class EnhancedHREvaluator:
    ...
    def evaluate(self, user_answer: str, question: str, expected_answer: Optional[str] = None) -> Dict:
        ...
        # STEP 4: HR-SPECIFIC SCORING COMPONENTS (4 components)
        ...
        # similarity*0.4 + content*0.3 + structure*0.1 + confidence*0.1 + sentiment*0.1
        final_score = (
        ...
        # STEP 8: OUTPUT FORMAT with 4 components
        return {
            "score": final_score_100,
            "feedback": self._generate_feedback(final_score_100, content_quality, generic_penalty),
            "confidence_level": 'High' if final_score_100 >= 80 else 'Medium' if final_score_100 >= 60 else 'Low',
            "components": {
                "content": {"score": round(content_quality * 100)},
                "confidence": {"score": round(confidence * 100)},
                "structure": {"score": round(structure * 100)},
                "sentiment": {"score": round(sentiment * 100)}
            },
            "metrics": {
    ...
    def _evaluate_content_quality(self, text: str, text_lower: str, processed: str) -> float:
        """Component 1: CONTENT QUALITY (30% weight)."""
        score = 0.0
        words = text.split()
...
if __name__ == '__main__':
    ...
    if not raw.strip():
        print(json.dumps({
            ...
            "components": {},
            "metrics": {"content": 0, "confidence": 0, "structure": 0, "sentiment": 0, "similarity": 0}
        }))
        sys.exit(0)
